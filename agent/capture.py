...
This module refactors the *functional* parts of a typical Part-1 `capture_logger.py`
into reusable pieces for Milestone-2, while aligning with our package structure:
...
Where the original Part-1 script printed per-packet lines, Milestone-2 now:
- uses detectors to emit **detections** to `logs/detections.jsonl`, and
- writes **ops** (start/shutdown/errors) to `logs/ops.jsonl`.

If you still want per-packet logging (for teaching/demo), set `capture.log_every_packet: true`
...
class LiveAdapter:
    ...
    def __enter__(self):
        # NOTE: Students may add display_filter, bpf filters, or tshark args here.
...
def normalize_basics(pkt) -> dict:
    ...
    TODO(student): extend as you add protocols or libraries.
    """
    d = {
        "ts": _now_iso(),
        "src": None,
    ...
    try:
        ...
        # We keep it defensive: normalization must never crash packet processing.
        pass
