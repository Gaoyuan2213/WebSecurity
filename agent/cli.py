...
STUDENT TODOs are marked clearly below. Focus points:
- Plug in new detectors (Week-3+)
- Add per-protocol normalization if you switch libraries
- Add more subcommands (e.g., `replay`, `bench`) if desired
"""
...
def process_packet(cfg, pkt):
    ...
    TODO(student):
    - If you change libraries or add protocols, normalize the fields you need.
    - Keep it defensive: use hasattr/getattr instead of assuming attributes exist.
    """
    try:
        ...
        # TODO(student): Add your new detectors below in future milestones.
        # Example:
        # if cfg["rules"].get("tls_sni_blocklist"):
        #     detect_tls_sni(cfg, pkt, src, dst)
    ...
    try:
        with cap.FileAdapter(args.pcap) as reader:
            for pkt in reader.stream():
                if STOP["flag"]:
                    # Student feedback fix: break out of the packet loop on stop
                    break
...
def main(argv=None):
    ...
    CLI definition.
    TODO(student):
      - Add new subcommands if you extend functionality (e.g., `replay`, `bench`).
      - Consider `--rules` or `--threshold` overrides for advanced use (optional).
    """
